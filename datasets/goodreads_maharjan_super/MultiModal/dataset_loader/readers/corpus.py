...
class Corpus:
    ...
    #     kls = cls(reader,label_extractor) # creates an instance of the class?
                ...
                for x, y in zip(X, Y):
                    if x.book_id not in books_added:
                        if x.book_id in data['test']:
                            ...
                            print('Book id {} not in train, val, and test set'.format(x.book_id))
                kls.X_train, kls.Y_train = np.array(X_train), np.array(Y_train)
                kls.X_val, kls.Y_val = np.array(X_val), np.array(Y_val)
                kls.X_test, kls.Y_test = np.array(X_test), np.array(Y_test)

                print("Total unique books: {}".format(len(books_added)))
                print("Training instances {}, Val instances {}, Test instances {}".format(kls.X_train.shape, kls.X_val.shape, kls.X_test.shape))
            except yaml.YAMLError as exc:
                print(exc)
                raise OSError("Error reading the split file")
        return kls
